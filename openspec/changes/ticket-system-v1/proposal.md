...
- **Supporting**: simulated users with a switcher, categories back office, edit and soft delete while `open`, release to queue.
...
| Real login and user management | Not part of the stated problem; seeded users are enough to demonstrate API-enforced roles |

## Product Rules

### Access

- 4 requesters and 4 agents are preloaded; each user has exactly one role. A UI switcher picks the acting user; there is no login.
...
- `user-access`: seeded users, user switcher, role and ownership checks.
...
- **Acting user**: the web app sends the switcher's user id with every request; the API loads that user and checks role and ownership before any action.
...
|---|---|---|
| `api/src/` | New | Feature modules, acting-user checks, seed |
...
| `web/src/` | New/Modified | Switcher, requester and agent views, back office, dashboard (`App.tsx` becomes the shell) |
...
- Existing scaffold only. No new runtime dependency planned; any addition needs a stated reason in design.
...
**Also confirmed**: comments cannot be edited or deleted; every comment is visible to the ticket's requester (no internal notes); agents do not create tickets; resolving needs no resolution note; reopen and real login stay out of scope for the reasons above.
