...
- [x] 1.6 `users/`: `user.schema.ts`, `user.dto.ts`, `users.seed.ts` (`bulkWrite` seed, 4 requesters + 4 agents, fixed ids, `$setOnInsert`), `users.service.ts` (queries), `users.controller.ts` (`GET /users`, `@Public`), `users.module.ts`.
...
- [ ] 2.2 RED→GREEN `ticket-categories.e2e-spec.ts` (seed + CRUD): "Starter categories exist after startup and are editable"; "Requester cannot create a category"; "Creating a category with a name that already exists fails"; seed idempotency. Also add "A requester attempting an agent-only action is rejected" to `user-access.e2e-spec.ts` (requester calls `POST /categories`). Implement `categories.seed.ts` (seed 4 starters when empty), `categories.service.ts` (create/list/edit/delete, `markUsed()`, conditional `used: false` filters), `categories.controller.ts`, `categories.module.ts`; register in `app.module.ts`.
