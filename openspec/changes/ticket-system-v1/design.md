...
A NestJS modular monolith with one module per capability (`users`, `categories`, `tickets`, `dashboard`) over three MongoDB collections, plus a React SPA with no router, data or UI library, whose forms use react-hook-form with zod. The ticket is the aggregate: its audit history (comments included) is embedded, and every lifecycle action is one conditional `findOneAndUpdate` that checks the rule, changes the state and appends the event atomically. A global guard resolves the acting user from an `X-User-Id` header. The specs (written in parallel) own behavior; this document owns the HOW.
...
| `main.ts` | CORS, Swagger with the `X-User-Id` api-key scheme, listen |
...
```
Web ─POST /tickets/:id/take + X-User-Id─▶ ActingUserGuard ─▶ 401 unknown user / 403 not agent
...
`App.tsx` loads users, keeps the acting user in React state (default: first requester), renders the switcher and the role layout: requester → `tickets/RequesterTickets`; agent → tabs `AgentTickets`, `categories/CategoriesAdmin`, `dashboard/Dashboard`. Folders mirror API modules. `api.ts` is the only module that calls `fetch`.

## 2. Data Model

```ts
// users: seeded, immutable in v1
{ _id: 'requester-1'..'requester-4' | 'agent-1'..'agent-4', name: string, role: 'requester' | 'agent' }
...
| Ticket and event → user | Reference by id | 8 immutable users that the web already loads for the switcher | Responses carry ids, not names |
...
## 4. Acting User and Status Codes

- **Web**: `createApiClient(userId)` adds `X-User-Id` to every request; switching users creates a new client.
- **API**: `ActingUserGuard` reads the header, loads the user with `UsersService.findById`, checks `@Roles(...)` (no decorator = any known user) and sets `request.user`. `@Public()` skips it, used only by `GET /users`. Ownership and assignee checks live in `ticket-rules.ts` because they need the ticket.
- **Swagger**: `addApiKey({ type: 'apiKey', in: 'header', name: 'X-User-Id' }, 'acting-user')` plus `@ApiSecurity('acting-user')` on guarded controllers, so Authorize sets the user once.

| Case | Status |
|---|---|
| Missing header or unknown user | 401 |
...
|---|---|---|---|---|
| GET /users | public | none | 200 `UserDto[]`, requesters first | none |
...
| View | Contents |
|---|---|
| Switcher (`App.tsx`) | `<select>` of the 8 users labeled with role; the role layout is keyed by user id, so switching resets view state |
...
- **API client**: `listUsers()` plus `createApiClient(userId)` with one typed function per endpoint; non-2xx throws `ApiError(status, message)`. Components receive the client as a prop, so tests pass a fake client instead of mocking `fetch`. `types.ts` mirrors the DTOs by hand.
...
- **URLs and CORS**: `import.meta.env.VITE_API_URL ?? 'http://localhost:3000'`. The browser runs on the host in dev and Docker, and compose publishes the API on port 3000, so `compose.yaml` and the nginx image stay unchanged. The API calls `app.enableCors({ origin: process.env.WEB_ORIGIN ?? 'http://localhost:5173' })`; the preflight allows `X-User-Id`.
...
|---|---|---|
| API unit, `src/**/*.spec.ts`, no database | `pnpm run check` | `ticket-rules`: role, owner, assignee, state and deleted checks per action, and the edit diff. `ActingUserGuard`: 401, 403, `@Public`. `computeDashboard`: counts, medians (odd, even, empty), first take after a release, time in state reset only by state events |
| API e2e, `test/*.e2e-spec.ts`, real MongoDB, supertest on `AppModule` | `pnpm -C api test:e2e` | One file per capability. Authorization matrix (missing or unknown user, wrong role, wrong owner, not assignee); lifecycle and every 409; soft delete (404, hidden from lists and dashboard); exactly one history event per action with actor and time; category lock (current, previous, deleted ticket) and duplicate names; seeded data and seed idempotency. **Concurrent take**: the 4 agents take one ticket with `Promise.all` → exactly one 200, three 409, one `taken` event. **Category race**: concurrent delete category and create ticket never both succeed |
| Web, `*.test.tsx`, jsdom, fake client | `pnpm run check` | Switcher and role layouts, grouping by state with assignee, actions by role and state, timeline, category controls disabled when used, dashboard figures, `formatDuration`, `api.ts` header and error mapping. Forms (`TicketForm`, `CommentForm`, `CategoryForm`): empty or too-long input shows the field message and does not call `onSubmit`; valid input calls it; a rejected `onSubmit` shows the API message. Tests use Testing Library `fireEvent` and `findBy*` queries, since react-hook-form validates asynchronously (no `user-event` dependency) |
...
|---|---|---|
| `api/package.json` | Modify | Add `class-validator` and `class-transformer` (required by `ValidationPipe`) |
...
| `web/src/App.tsx`, `App.test.tsx` | Modify | Shell, switcher, agent tabs |
...
| `README.md`, `DECISIONS.md`, `.agents/context/domain.md`, `.agents/context/data-model.md` | Create | Run steps, seeded users, Swagger usage; decisions (section 11); domain; data model |
...
| Acting user | `X-User-Id` + global guard with `@Public` opt-out | Query parameter; JWT | Protected by default, Swagger Authorize works; the guard is the single swap point for real login |
| User ids | Readable strings (`agent-1`) | ObjectIds | Usable as-is in Swagger, curl and tests; id types differ across collections |
...
- A replica set enables transactions and change streams (notifications); real login replaces the `X-User-Id` guard.
