...
`App.tsx` reads the token from `localStorage` and asks `GET /auth/me`: without a valid token it renders `auth/LoginPage`; with one it renders a logout button (which discards the token) and the role layout: `tickets/TicketsPage` for both roles, plus `categories/CategoriesPage` for agents. React Router owns the URL, so the board's filters, sorting and page live in the query string and a ticket has its own address (`/tickets/TCK-142`): a filtered queue is a link an agent can send. Folders mirror API modules. `api.ts` is the only module that calls `fetch`.
...
| View                                                                           | Contents                                                                                                                                                                                                                                                                      |
| ------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Shell (`App.tsx`) and `auth/LoginPage.tsx`                                     | Login form (username and password) without a valid token; otherwise the user's name, a logout button and the role layout, keyed by user id so a new login resets view state                                                                                                   |
| `tickets/TicketsPage.tsx`, `components/Board.tsx`, `components/TicketCard.tsx` | One board for both roles, laid out like Jira: a column per state with a count, cards with code, title, category chip, requester (agents only), initials for the assignee and the age; the whole card links to the ticket. A requester gets the new-ticket `TicketForm` inline |
| `tickets/TicketDetailPage.tsx`                                                 | Code, state, title, who opened it and who has it; edit (`TicketForm`) and delete (two clicks) for the owner while open; take for any agent while open, release and resolve for the assignee; a refused transition shows the API's message and reloads; the `Timeline`         |
...
| `web/src/features/tickets/` `TicketsPage`, `TicketDetailPage`, `components/Board`, `components/TicketCard`, `components/TicketForm`, `components/Timeline`, `components/StateBadge`, `components/CommentForm` (`.tsx`, with tests)                | Create | Board, detail, forms, timeline                                                                                        |
| `web/src/tickets/validation.ts`                                                                                                                                                                                                                   | Create | zod schemas for ticket and comment forms, mirroring the API DTO limits                                                |
| `web/src/features/categories/CategoriesPage.tsx`, `CategoryForm.tsx`, `web/src/features/tickets/TicketFilters.tsx` (with tests)                                                                                                                   | Create | Back office with one form for create and rename; the board's filter bar                                               |
